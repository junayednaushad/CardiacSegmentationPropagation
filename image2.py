...
def load_img2(path, grayscale=False, target_size=None, pad_to_square=True,
...
def load_img3(path, grayscale=False, target_size=None, pad_to_square=True):
    """Loads an image into PIL format.
    # Arguments
...
class ImageDataGenerator2(object):
    ...
    def flow_from_path_list(self, path_list,
...
class NumpyArrayIterator(Iterator):
    ...
    # Arguments
        ...
        image_data_generator2: Instance of `ImageDataGenerator2`
    ...
    def __init__(self, x, y, image_data_generator2,
...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, image_data_generator2,
                 target_size=(256, 256), color_mode='rgb',
                 classes=None, class_mode='categorical',
                 batch_size=32, shuffle=True, seed=None,
                 data_format=None,
...
class PathListIterator(Iterator):
    """Iterator capable of reading images from a list of pathes.
    # Arguments
        path_list: a list of strings of pathes of the the images to be read.
            Each subdirectory in this directory will be
            considered to contain images from one class,
            or alternatively you could specify class subdirectories
            via the `classes` argument.
        image_data_generator2: Instance of `ImageDataGenerator2`
            to use for random transformations and normalization.
        target_size: tuple of integers, dimensions to resize input images to.
        pad_to_square: Boolean, whether to pad one such that the read image
            becomes a square, before any other augmentation such as resizing to
            target_size, linear transformation, etc.
        resize_mode: One of `"nearest"`, `"bilinear"`. Resize mode to resize PIL image.
        histogram_based_preprocessing: Boolean, whether to clip off extreme percentile pixels
        clahe: Boolean, whether to apply CLAHE
        color_mode: One of `"rgb"`, `"grayscale"`. Color mode to read images.
        class_list: Optional list of strings, names of classes for 
            images in list. Its length should be equal to that of list.
            It will be set automatically if not set.
        class_mode: Mode for yielding the targets:
            `"binary"`: binary targets (if there are only two classes),
            `"categorical"`: categorical targets,
            `"sparse"`: integer targets,
        ...
        save_format: Format to use for saving sample images
            (if `save_to_dir` is set).
        save_period: Integer, save one batch out of every how many batches.
    """

    def __init__(self, path_list, image_data_generator2,
