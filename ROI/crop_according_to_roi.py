...
sys.path.append('C:\\Users\\mpnau\\Documents\\ml_programs\\CardiacSegmentationPropagation\\Preprocessing')
...
def crop_according_to_roi():
    ...
    for k in used_statistics:
        ...
        eid_path = os.path.join(data_dir, eid)
        ...
        labels = [instant0_path, instant1_path]
        images = [x[:-10] + '.nii.gz' for x in labels]
        ...
        for i, image in enumerate(images):
            ...
            if i == 0:
                crop_image_file = os.path.join(eid_path, 'ed_crop_sa.nii.gz')
            else:
                crop_image_file = os.path.join(eid_path, 'es_crop_sa.nii.gz')
            nib.save(nib.Nifti1Image(crop_image_data, np.eye(4)), crop_image_file)

        for i, label in enumerate(labels):
            label_load = nib.load(label)
            label_data = label_load.get_fdata()
            ...
            if i == 0:
                crop_label_file = os.path.join(data_dir, str(eid), 'ed_crop_label_sa.nii.gz')
            else:
                crop_label_file = os.path.join(data_dir, str(eid), 'es_crop_label_sa.nii.gz')
        ...
        crop_images = [os.path.join(eid_path, 'ed_crop_sa.nii.gz'), os.path.join(eid_path, 'es_crop_sa.nii.gz')]
        ed_es_times = [ed_es_instant0, ed_es_instant1]
        for crop_image_file, t in zip(crop_images, ed_es_times):
            crop_image_data = nib.load(crop_image_file).get_fdata()
            ...
            for s in range(slices):
                ...
                Image.fromarray((np.rot90(crop_image_data[:, ::-1, s], 3) * multiplier).astype('uint8')).save(s_t_image_file)

    
        # Save cropped 2D labels
        crop_labels = [os.path.join(eid_path, 'ed_crop_label_sa.nii.gz'), os.path.join(eid_path, 'es_crop_label_sa.nii.gz')]
        for crop_label_file, t in zip(crop_labels, ed_es_times):
            crop_label_data = nib.load(crop_label_file).get_fdata()

            for s in range(slices):
                    s_t_label_file = os.path.join(crop_2D_path, 'crop_2D_gt_{}_{}.png'.format(str(s).zfill(2), str(t).zfill(2)) )
                    Image.fromarray((np.rot90(crop_label_data[:, ::-1, s], 3) * 50).astype('uint8')).save(s_t_label_file)
